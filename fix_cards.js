...
const file = path.join(__dirname, 'frontend/src/app/components/dashboard/dashboard.component.html');
let content = fs.readFileSync(file, 'utf8');

const oldCard = `            <div *ngFor="let target of getTargetsByCategoryAndType(sede, type)" cdkDrag (click)="openDetails(target)"
              class="bg-card rounded-[2rem] border border-theme shadow-md hover:shadow-[0_0_40px_-8px_rgba(59,130,246,0.25)] hover:border-blue-500/20 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer group overflow-hidden">`;

const newCard = `            <div *ngFor="let target of getTargetsByCategoryAndType(sede, type)"
              ...
              <div class="flex items-center w-full pl-5 pr-3 py-[14px] md:pl-6 md:pr-4 md:py-4 gap-3 md:gap-5">
                ...
                <div class="relative flex-shrink-0">
                  <div class="w-2 h-2 rounded-full"
                    ...
                    [style.box-shadow]="target.alive ? '0 0 6px rgba(96,165,250,0.9)' : '0 0 6px rgba(239,68,68,0.9)'"></div>
...
// Find the old card start
const startIdx = content.indexOf(oldCard);
if (startIdx === -1) {
    // Try CRLF version
    const crlfOld = oldCard.replace(/\n/g, '\r\n');
    const crlfIdx = content.indexOf(crlfOld);
    console.log('CRLF match:', crlfIdx !== -1);
    if (crlfIdx !== -1) {
        // Find the end of the old card block (closing </div> after desktop actions)
        const oldDesktop = `              <!-- Acciones Desktop -->`;
        const desktopIdx = content.indexOf(oldDesktop, crlfIdx);
        const closingTag = `            </div>\r\n\r\n            <!-- Drag overlap -->`;
        const endIdx = content.indexOf(closingTag, desktopIdx);
        if (endIdx !== -1) {
            const before = content.substring(0, crlfIdx);
            const after = content.substring(endIdx + `            </div>`.length);
            content = before + newCard.replace(/\n/g, '\r\n') + after;
            fs.writeFileSync(file, content, 'utf8');
            console.log('Done! CRLF version replaced.');
        } else {
            console.error('End tag not found after desktop actions');
        }
    } else {
        console.error('Old card pattern not found. First 50 chars of file:', JSON.stringify(content.substring(3000, 3100)));
    }
} else {
    console.log('LF match found at', startIdx);
}
